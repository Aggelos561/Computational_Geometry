...
Segment_2 chooseVisibleSegment(std::vector<Segment_2> &visibleSegments, Point &nextPoint, ft &area) {
  ...
  std::map<ft, Segment_2> areaToSegMap;

  for (Segment_2 segment : visibleSegments){

    ft newArea = getTriangleArea(segment, nextPoint);

    if (areaToSegMap.find(newArea) == areaToSegMap.end())
      areaToSegMap.insert({newArea, segment});
  }

  // ft minArea = std::numeric_limits<ft>::max();
  // for (auto i : areaToSegMap)
  //   if (i.first < minArea) {
  //     minArea = i.first;
  //   }

  // area += minArea;
  // return areaToSegMap[minArea];
...
int main() {
  ...
  ft area = CGAL::area(polygLinePoints[0], polygLinePoints[1], polygLinePoints[2]);
  ...
  while (remainingPoints.size() > 0) {
    ...
    while(true){
      ...
      // if NO red segments where found then the next point is ON the polygon
